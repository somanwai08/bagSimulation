...
const requireContext = require.context(
  '../../../assets/images/woodhandle/for Simulation/',
  false
)
const whImages = requireContext.keys().map(requireContext)

console.log(whImages, 'whImages')
...
export default function FinalBag() {
  ...
  // 選擇gem時變化的副作用
  useEffect(() => {
    const arr1 = gemImages1
      .map((item) => item.split('media/')[1])
  ...
  useEffect(() => {
    if (selectedSize === 'PM' && selectedHandle === 'YUBARI') {
      ...
      selectedSize === 'GM' &&
      selectedHandle === 'WISHBONE Platanus Wood' &&
      (selectedColor === 'Bleu' ||
        selectedColor === 'Precious Skins-Emerald' ||
        selectedColor === 'Precious Skins-Gold Black' ||
      ...
      selectedSize === 'GM' &&
      selectedHandle === 'WISHBONE Olive Wood' &&
      (selectedColor === 'Bleu' ||
        selectedColor === 'Precious Skins-Emerald' ||
        selectedColor === 'Precious Skins-Gold Black' ||
        selectedColor === 'Turquoise')
